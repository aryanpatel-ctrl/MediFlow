...
const { sendFCMNotification, sendMulticastFCMNotification, sendToTopic } = require('../config/firebase');
...
async function sendPushNotification(token, { title, body, data }) {
  ...
  if (result.shouldRemoveToken) {
    try {
      ...
      console.log('Removed invalid FCM token from user');
    } catch (err) {
...
async function sendBulkPushNotification(userIds, { title, body, data }) {
  ...
  if (tokens.length === 0) {
    return { sent: false, error: 'No valid tokens' };
  }

  const result = await sendMulticastFCMNotification(tokens, { title, body }, data || {});
 ...
 */
async function sendHospitalBroadcast(hospitalId, { title, body, data }) {
  return sendToTopic(`hospital_${hospitalId}`, { title, body }, data || {});
}
